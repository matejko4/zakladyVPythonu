...
formatovany_cas = datum.strftime("%d.%m.%Y %H:%M:%S")
print(f"Řešení 1: {formatovany_cas}")
...
for i in range(1,4):
    date = easter(datetime.datetime.now().year + i, EASTER_WESTERN)
    print(f"Řešení 2: Velikonoce za {i} let: {date.day}.{date.month}")


today = datum.date()
year = rrule(YEARLY, dtstart=datum, bymonth=12, bymonthday=24, byweekday=SU)[0].year
print(f"Řešení 3: Rok ve kterem bude stedry den v nedeli je {year}")
